...
async function deleteAction(id) {
  ...
  async function upsertTodo(input) {
    ...
    let linked = null;
    if (rec.actionId) linked = actions.find(a => !a._deleted && a.id === rec.actionId) || null;

    if (!linked) {
      linked = await upsertAction({
        title: rec.text,
  ...
  async function deleteHabitCompletion(id) { await markDeleted(STORES.habitCompletions, id); }
